...
void mg_http_serve_file(struct mg_connection *c, struct mg_http_message *hm,
  ...
  bool gzip = false;
  // Ensure the root directory and path are not NULL
  const char *root_dir = opts->root_dir ? opts->root_dir : "";
  const char *rel_path = path ? path : "";

  // Calculate the length of the final path
  size_t root_len = strlen(root_dir);
  size_t rel_len = strlen(rel_path);
  size_t total_len = root_len + rel_len + 1;

  // Define a buffer to hold the combined path
  char full_path[500];

  // Initialize the buffer with root_dir
  strcpy(full_path, root_dir);

  // Ensure there is a '/' between root_dir and rel_path if not present
  full_path[root_len] = '\0'; // Null-terminate after adding '/'
